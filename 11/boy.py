	...
	def draw(boy):
		if boy.velocity == 1:
			boy.image.clip_composite_draw(boy.frame * 100, 300, 100, 100,
										  math.radians(90), '', boy.x - 25, boy.y - 25, 100, 100)
		else:
			boy.image.clip_composite_draw(boy.frame * 100, 300, 100, 100,
										  math.radians(90), '', boy.x - 25, boy.y - 25, 100, 100)
...
next_state_table = {
	...
	DashState: {RIGHT_UP: IdleState, LEFT_UP: IdleState,
				LEFT_DOWN: IdleState, RIGHT_DOWN: IdleState,
				DASH_DOWN: RunState, DASH_UP: RunState,
				DASH_TIMER: RunState},
		...
		for event in events:
			...
			if event.type == SDL_KEYDOWN or event.type == SDL_KEYUP:
				if event.key == SDLK_RIGHT or event.key == SDLK_LEFT:
					boy.add_event(key_event_table[(event.type, event.key)])
				if (event.key == SDLK_LSHIFT or event.key == SDLK_RSHIFT) and (
						boy.cur_state == DashState or boy.cur_state == RunState):
					boy.add_event(key_event_table[(event.type, event.key)])
		boy.update()
